...
const oldHeaderRegex = /\{\/\* A4 Header \*\/\}\s*<div style=\{\{ display: 'flex', justifyContent: 'space-between'[\s\S]*?\{\/\* A4 Body \(Table\) \*\/\}/;
...
const tableRegex = /<table style=\{\{ width: '100%', borderCollapse: 'collapse', marginTop: 20 \}\}>\s*<thead>\s*<tr>\s*\{Object\.keys\(reportData\[0\] \|\| \{\}\)\.map\(\(col, i\) => \(\s*<th key=\{i\}>\{col\.replace\(\/_.*?\)\}\s*<\/tr>\s*<\/thead>\s*<tbody>\s*\{reportData\.map\(\(row, idx\) => \(\s*<tr key=\{idx\}>\s*\{Object\.entries\(row\)\.map\(\(\[key, val\], i\) => \{[\s\S]*?return \([\s\S]*?<\/td>\s*\);\s*\}\)\}\s*<\/tr>\s*\)\)\}\s*<\/tbody>\s*<\/table>/;
// Let's use indexOf instead to be absolutely safe.
...
console.log('UI injected safely.');
