new_admin_js = """const express = require('express');
...
router.post('/atletas', verificarAcesso, async (req, res) => {
    ...
    try {
        ...
        `;
        const r = await pool.query(query, [nome, categoria_id, posicao, nome_responsavel, telefone_responsavel, status_medico]);
...
router.put('/atletas/:id', verificarAcesso, async (req, res) => {
    ...
    try {
        ...
        `;
        const r = await pool.query(query, [nome, categoria_id, posicao, nome_responsavel, telefone_responsavel, status_medico, id]);
...
router.post('/treinadores', verificarAdmin, async (req, res) => {
    ...
    try {
        ...
        const q1 = "INSERT INTO treinadores (nome, usuario, usuario_lc, senha_hash, perfil, precisa_trocar_senha) VALUES ($1, $2, $3, $4, $5, true) RETURNING id";
        const resT = await client.query(q1, [nome, usuario_lc, usuario_lc.toLowerCase(), senhaHash, perfil]);
