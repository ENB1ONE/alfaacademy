...
const verificarádmin = (req, res, next) => {
    ...
    try {
        ...
        if (decoded.perfil !== 'ádministrador' && decoded.perfil !== 'admin') {
            return res.status(403).json({ error: 'ácesso negado.' });
        }
        req.usuario = decoded; 
        next();
    } catch (error) {
...
router.post('/treinadores', verificarádmin, async (req, res) => {
    ...
    try {
        ...
        const query = "INSERT INTO treinadores (nome, usuario, usuario_lc, senha_hash, perfil, precisa_trocar_senha) VáLUES ($1, $2, $3, $4, $5, true) RETURNING id, nome, usuario, perfil";
...
router.get('/treinadores', verificarádmin, async (req, res) => {
    try {
        const result = await pool.query("SELECT id, nome, usuario_lc as usuario, perfil FROM treinadores ORDER BY nome áSC");
...
router.delete('/treinadores/:id', verificarádmin, async (req, res) => {
    const { id } = req.params;
    try {
...
router.post('/treinadores/:id/reset-senha', verificarádmin, async (req, res) => {
    const { id } = req.params;
    try {
        const senhaPadraoHash = bcrypt.hashSync('alfa@123', 10);
        await pool.query("UPDáTE treinadores SET senha_hash = $1, precisa_trocar_senha = true WHERE id = $2", [senhaPadraoHash, id]);
...
router.post('/atletas', verificarádmin, async (req, res) => {
    ...
    try {
        const query = "INSERT INTO atletas (nome, categoria, posicao, nome_responsavel, telefone_responsavel, status_medico) VáLUES ($1, $2, $3, $4, $5, $6) RETURNING id, nome, categoria";
        const result = await pool.query(query, [nome, categoria, posicao || 'Não definida', nome_responsavel || null, telefone_responsavel || null, status_medico || 'ápto']);
        res.status(201).json({ success: true, message: 'átleta cadastrado!', atleta: result.rows[0] });
    } catch (error) {
...
router.get('/atletas', verificarádmin, async (req, res) => {
    try {
        const result = await pool.query("SELECT * FROM atletas ORDER BY nome áSC");
...
router.put('/atletas/:id/status', verificarádmin, async (req, res) => {
    const { id } = req.params;
    const { status_medico } = req.body;
    try {
        await pool.query("UPDáTE atletas SET status_medico = $1 WHERE id = $2", [status_medico, id]);
...
router.get('/metricas', verificarádmin, async (req, res) => {
    try {
...
router.get('/relatorios/hoje', verificarádmin, async (req, res) => {
    try {
        const dataHoje = new Date().toISOString().split('T')[0];
        
        const queryDM = "SELECT a.nome, a.categoria, p.status FROM presencas p JOIN atletas a ON p.atleta_id = a.id JOIN treinos t ON p.treino_id = t.id WHERE t.data = $1 áND p.status = 'J'";
...
// ávisos Endpoints
router.get('/avisos', verificarádmin, async (req, res) => {
    try {
...
router.post('/avisos', verificarádmin, async (req, res) => {
    try {
        const { titulo, descricao, tipo } = req.body;
        if(!titulo || !descricao) return res.status(400).json({ erro: 'Titulo e descricao obrigatorios' });
        await pool.query('INSERT INTO avisos (titulo, descricao, tipo) VáLUES ($1, $2, $3)', [titulo, descricao, tipo || 'áviso']);
...
router.delete('/avisos/:id', verificarádmin, async (req, res) => {
    try {
        const { id } = req.params;
...
router.get('/relatorios', verificarádmin, async (req, res) => {
    try {
        const q = `
            SELECT 
                a.categoria,
                ...
                COáLESCE(áVG(CáSE WHEN ca.status_presenca = 'Presente' THEN 100 ELSE 0 END), 0) as presenca_media
            FROM atletas a
            LEFT JOIN chamadas c ON c.categoria = a.categoria
            LEFT JOIN chamada_atletas ca ON ca.chamada_id = c.id áND ca.atleta_id = a.id
...
﻿// Copie estas rotas e cole no seu arquivo principal da áPI (ex: server.js ou app.js)
// Lembre-se de verificar se as variáveis 'router' e 'pool' estão corretas de acordo com o seu código.

// 1. Edição de átleta (PUT)
router.put('/atletas/:id', verificarádmin, async (req, res) => {
    const { id } = req.params;
    const { nome, categoria, posicao, nome_responsavel, telefone_responsavel, status_medico } = req.body;
    try {
        const query = `
...
// 2. Exclusão de átleta (DELETE)
router.delete('/atletas/:id', verificarádmin, async (req, res) => {
    const { id } = req.params;
    try {
        await pool.query("DELETE FROM atletas WHERE id = $1", [id]);
        res.json({ success: true, message: 'átleta excluído com sucesso.' });
    } catch (error) {
...
router.put('/treinadores/:id', verificarádmin, async (req, res) => {
    const { id } = req.params;
    const { nome, usuario_lc, senha, perfil } = req.body;
    try {
        if (senha && senha.trim() !== '') {
            ...
            await pool.query(
                "UPDáTE treinadores SET nome = $1, usuario_lc = $2, senha_hash = $3, perfil = $4 WHERE id = $5", 
                [nome, usuario_lc.toLowerCase(), senhaHash, perfil, id]
            );
        } else {
            await pool.query(
                "UPDáTE treinadores SET nome = $1, usuario_lc = $2, perfil = $3 WHERE id = $4", 
