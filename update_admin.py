...
// NOVO: Exclusão de treinador
router.delete('/treinadores/:id', verificarAdmin, async (req, res) => {
    const { id } = req.params;
    try {
        await pool.query("DELETE FROM treinadores WHERE id = ", [id]);
...
router.post('/treinadores/:id/reset-senha', verificarAdmin, async (req, res) => {
    ...
    try {
        ...
        await pool.query("UPDATE treinadores SET senha_hash = , precisa_trocar_senha = true WHERE id = ", [senhaPadraoHash, id]);
