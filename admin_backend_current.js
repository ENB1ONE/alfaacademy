...
const verificarAcesso = (req, res, next) => {
    ...
    try {
        ...
        return res.status(403).json({ error: 'Sess뿯½o inv뿯½lida ou expirada.' });
    }
};
...
router.get('/metricas', verificarAcesso, async (req, res) => {
    try {
        ...
        let dmQ = "SELECT COUNT(*) FROM atletas WHERE status_medico = 'Lesionado' OR status_medico = 'Transi뿯½뿯½o'";
        let vals = [];
        ...
        if (req.usuario.perfil === 'Treinador') {
            ...
            dmQ = `SELECT COUNT(*) FROM atletas a JOIN treinador_categoria tc ON a.categoria_id = tc.categoria_id WHERE tc.treinador_id = $1 AND (status_medico = 'Lesionado' OR status_medico = 'Transi뿯½뿯½o')`;
...
// ==========================================
// HIST뿯½RICO DE CHAMADAS E RELAT뿯½RIOS
// ==========================================
...
router.get('/treinos/:id/presencas', verificarAcesso, async (req, res) => {
    try {
        const { id } = req.params;
        // Valida뿯½뿯½o de acesso do treinador
        if (req.usuario.perfil === 'Treinador') {
...
router.post('/eventos', verificarAcesso, async (req, res) => {
    ...
    try {
        ...
        registrarLog(req.usuario.id, `Agendou evento ${eventTitle} para M뿯½ltiplas Categorias`, null);
        res.json({ success: true });
    } catch (e) {
...
// ==========================================
// RELAT뿯½RIO DE FREQU뿯½NCIA GERAL
// ==========================================
...
router.get('/jogos/:id/convocados', verificarAcesso, async (req, res) => {
    try {
        ...
        if (gameRes.rows.length === 0) return res.status(404).json({error: 'Jogo n뿯½o encontrado'});
...
router.post('/jogos/:id/convocacao', verificarAcesso, async (req, res) => {
    try {
        ...
        registrarLog(req.usuario.id, `Atualizou convoca뿯½뿯½o do jogo ID ${id}`, { atletas: atletas_ids });
        res.json({ success: true });
    } catch (e) {
