...
// 1. Replace the className="section-card" with "bloco-partida" in the jogos block
const oldDiv = `<div className="section-card" key={\`det-\${idx}\`} style={{ marginTop: '25px', breakInside: 'avoid', pageBreakInside: 'avoid' }}>`;
const newDiv = `<div className="bloco-partida" key={\`det-\${idx}\`} style={{ marginTop: '25px' }}>`;
code = code.replace(oldDiv, newDiv);

// 2. Patch the style block with the new strict rules requested by user
...
console.log('HTML and CSS patched successfully with zero margins and bloco-partida.');
