...
// Use indexOf to carefully slice the string to replace
let startIdx = code.indexOf('<div className="card" key={a.id}');
let endStr = "</span>\n                  </div>\n                </div>\n            </div>"; // The end of the card
let endIdx = code.indexOf(endStr, startIdx);

if (startIdx === -1 || endIdx === -1) {
    console.log("Could not find the card block boundaries.", startIdx, endIdx);
    process.exit(1);
}

const originalBlock = code.substring(startIdx, endIdx + endStr.length);
...
const newBlock = `<div className="card" key={a.id} style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '15px', border: '1px solid rgba(255,255,255,0.05)' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid rgba(255,255,255,0.05)', padding: '12px 0', paddingBottom: 0 }}>
                    ...
                    <span style={{ fontSize: '0.75rem', fontWeight: 'bold', textTransform: 'uppercase', padding: '4px 10px', borderRadius: '4px', background: a.status_medico === 'Lesionado' ? 'rgba(239, 68, 68, 0.1)' : 'rgba(34, 197, 94, 0.1)', color: a.status_medico === 'Lesionado' ? '#ef4444' : '#22c55e', border: \`1px solid \${a.status_medico === 'Lesionado' ? 'rgba(239, 68, 68, 0.2)' : 'rgba(34, 197, 94, 0.2)'}\` }}>{a.status_medico || 'APTO'}</span>
                  </div>
                </div>

                {/* Bottom Row: Actions */}
                <div style={{ display: 'flex', gap: '8px', borderTop: '1px solid rgba(255,255,255,0.05)', paddingTop: '15px', justifyContent: 'flex-end' }}>
                    <button onClick={() => navigate('/relatorios', { state: { triggerPresencasId: a.id, triggerPresencasNome: a.nome } })} title="Histórico de Presença" className="btn" style={{ padding: '8px 12px', background: 'rgba(248, 193, 70, 0.05)', color: 'var(--ouro)', border: '1px solid rgba(248, 193, 70, 0.2)', borderRadius: '8px', display: 'flex', alignItems: 'center', gap: '8px', flex: 1, justifyContent: 'center' }}><ClipboardCheck size={16} /> <span style={{fontSize: '0.85rem', fontWeight: 'bold'}}>Histórico</span></button>
