...
if (startIdx !== -1) {
    ...
    // EndIdx is the index of the VERY LAST '</div>' which corresponds to the wrapper.
    // However, it's safer to just split and join.
    let codeParts = code.split('{/* HIDDEN EXECUTIVE DASHBOARD REPORT */}');
    let beforeHidden = codeParts[0];
    
    // The previous hidden section ends exactly right before the final `    </div>\n  );\n}`
    // Let's just do a simple replacement of the whole old section.
    
    const finalCode = beforeHidden + newReportCode + `    </div>\n  );\n}\n`;
