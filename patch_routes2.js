...
`;
    ...
    // Just fix the syntax error if it was injected with missing variables
    content = content.replace("await pool.query('DELETE FROM avisos WHERE id = \\', [id]);", "await pool.query('DELETE FROM avisos WHERE id = $1', [id]);");
    content = content.replace("await pool.query('INSERT INTO avisos (titulo, descricao, tipo) VALUES (, , )'", "await pool.query('INSERT INTO avisos (titulo, descricao, tipo) VALUES ($1, $2, $3)'");
    fs.writeFileSync(p, content);
    console.log('Rotas reparadas.');
}
