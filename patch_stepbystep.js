...
// 1. Revert HTML to use only "bloco-jogo" (no nova-pagina logic)
const oldDivRegex = /<div className=\{`bloco-partida[^>]*key=\{`det-\$\{idx\}`\}[^>]*>/;
const newDiv = `<div className="bloco-jogo" key={\`det-\${idx}\`} style={{ marginTop: '25px' }}>`;
code = code.replace(oldDivRegex, newDiv);

// 2. Update html2pdf options to remove the .nova-pagina dependency
const pageBreakRegex = /pagebreak:\s*\{\s*mode:\s*\['css',\s*'legacy'\],\s*before:\s*\['\.nova-pagina'\],\s*avoid:\s*\['tr'\]\s*\}/g;
const newPageBreak = "pagebreak: { mode: ['css', 'legacy'], avoid: ['tr', '.bloco-jogo'] }";
code = code.replace(pageBreakRegex, newPageBreak);

// 3. Rewrite the CSS style block
...
console.log('Fixed PDF issues based on user step-by-step.');
