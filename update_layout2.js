...
// Close the grid and put button in its own flex container
code = code.replace(
    /<div style=\{\{\s*flex:\s*'1 1 200px',\s*display:\s*'flex'\s*\}\}>\s*<button className="btn" onClick=\{fetchGeneratorData\} disabled=\{loadingData\}/,
    `</div>
...
code = code.replace(
    /style=\{\{\s*width:\s*'100%',\s*background:\s*'var\(--ouro\)',\s*color:\s*'#000',\s*fontWeight:\s*'bold'\s*\}\}/,
    `style={{ padding: '12px 24px', background: 'var(--ouro)', color: '#111', fontWeight: 'bold', borderRadius: '8px', minWidth: '200px' }}`
);
