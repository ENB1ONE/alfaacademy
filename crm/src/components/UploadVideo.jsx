...
export default function UploadVideo({ onUploadSuccess }) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // Simula um tempo de processamento antes de transitar
      setTimeout(() => {
      ...
      console.error(error);
      alert('Ocorreu um erro no envio. Verifique a conexão com o servidor.');
      setIsUploading(false);
    }
  };
  ...
  return (
    <div className="card" style={{ maxWidth: '500px', margin: '40px auto', padding: '30px' }}>
      ...
      >
        {isUploading ? 'Enviando e Processando...' : 'Iniciar Análise'}
      </button>

      {isUploading && (
        <p style={{ textAlign: 'center', marginTop: '15px', color: 'var(--ouro)', fontSize: '13px' }}>
          Realizando scouting virtual... aguarde.
        </p>
      )}
    </div>
  );
